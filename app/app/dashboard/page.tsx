...
function DashboardPage() {
  ...
  async function saveStatus() {
    ...
    const oldStatus = selectedCase.status_name
    ...
    await supabase.from('case_logs').insert({ 
      case_id: selectedCase.id, author_id: profile.id, author_name: profile.full_name, 
      content: `🔄 סטטוס שונה: "${oldStatus}" → "${editStatus}"`,
  ...
  const greetingMsg = (() => {
    const parts: string[] = []
    if (!isAdmin) {
      if (overdueCases.length > 0) parts.push(`⚠️ ${overdueCases.length} פניות חרגו מ-SLA`)
      if (allIP.length > 0) parts.push(`⏳ ${allIP.length} פניות בטיפול`)
      if (doneMine > 0) parts.push(`✅ ${doneMine} טופלו`)
    } else {
      if (overdueCases.length > 0) parts.push(`⚠️ ${overdueCases.length} חריגות SLA`)
      if (todayCases.length > 0) parts.push(`📅 ${todayCases.length} פניות היום`)
    }
    return parts.length > 0 ? parts.join(' | ') : '✨ הכל תקין, יום טוב!'
  })()
    ...
    <>
      ...
      <div style={{ padding:'22px 26px', maxWidth:1600, margin:'0 auto' }}>
        ...
        <div className="page-header">
          <div>
            <div className="page-title">{greetWord}, {profile?.full_name} 👋</div>
            <div style={{ fontSize:13, color:'var(--text3)', marginTop:2 }}>{dateStr}</div>
            <div style={{ fontSize:12, marginTop:6, padding:'6px 12px', background: overdueCases.length > 0 ? '#fef2f2' : '#f0fdf4', borderRadius:8, display:'inline-block', color: overdueCases.length > 0 ? '#dc2626' : '#15803d', fontWeight:600 }}>
              {greetingMsg}
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={e => { if(e.target===e.currentTarget) setSelectedCase(null) }}>
          <div className="modal">
            ...
            {caseTab==='details' && <>
              ...
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text2)', textTransform:'uppercase', marginBottom:10 }}>📋 לוג פניה</div>
              <div style={{ marginBottom:12, maxHeight:220, overflowY:'auto' }}>
                {logs.length === 0
                  ? <div style={{ color:'var(--text3)', fontSize:12, textAlign:'center', padding:'1rem' }}>אין תיעודים</div>
                  : logs.map((l, idx) => {
                    const isStatus = l.log_type === 'status_change' || l.content?.startsWith('🔄')
                    const isSMS = l.content?.startsWith('📱')
                    const isFile = l.content?.startsWith('📎')
                    const isAuto = isStatus || isSMS || isFile
                    const icon = isStatus ? '🔄' : isSMS ? '📱' : isFile ? '📎' : '✏️'
                    const color = isStatus ? '#93c5fd' : isSMS ? '#86efac' : isFile ? '#fcd34d' : '#e5e7eb'
                    const bg = isStatus ? '#dbeafe' : isSMS ? '#dcfce7' : isFile ? '#fef3c7' : '#f1f5f9'
                    return (
                      <div key={l.id} style={{ display:'flex', gap:10, marginBottom:10 }}>
                        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', flexShrink:0 }}>
                          <div style={{ width:28, height:28, borderRadius:'50%', background:bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, border:`2px solid ${color}` }}>{icon}</div>
                          {idx < logs.length-1 && <div style={{ width:2, flex:1, background:'#e5e7eb', minHeight:12 }} />}
                        </div>
                        <div style={{ flex:1, paddingBottom:4 }}>
                          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:2 }}>
                            <span style={{ fontSize:12, fontWeight:700, color: isAuto ? '#6366f1' : '#2563eb' }}>{l.author_name}</span>
                            <span style={{ fontSize:10, color:'#9ca3af' }}>{new Date(l.created_at).toLocaleString('he-IL',{timeZone:'Asia/Jerusalem',day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</span>
                          </div>
                          <div style={{ fontSize:12, color:'#374151', background: isAuto ? '#f8faff' : 'transparent', padding: isAuto ? '4px 8px' : 0, borderRadius:6, borderRight: isAuto ? `3px solid ${color}` : 'none' }}>
                            {l.content}
                          </div>
                        </div>
                      </div>
