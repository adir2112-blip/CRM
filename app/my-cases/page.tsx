...
export default function MyCasesPage() {
  ...
  useEffect(() => {
    ...
    setFiltered(cases.filter(c => {
      ...
      if (q && !c.customer_name?.toLowerCase().includes(q) && !c.phone?.includes(q)) return false
      return true
    }))
  }, [search, fStatus, fDate, cases])
      ...
      <div style={{ padding:'22px 26px' }}>
        ...
        <div className="card card-pad" style={{ marginBottom:16 }}>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
            ...
            {(fStatus||fDate!=='all'||search) && <button className="btn btn-xs" onClick={() => { setFStatus(''); setFDate('all'); setSearch('') }}>✕ נקה</button>}
          </div>
        </div>
        ...
        <div className="card" style={{ padding:0 }}>
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>#</th><th>שם לקוח</th><th>טלפון</th><th>ארגון</th><th>סיווג</th><th>סטטוס</th><th>עודכן</th></tr>
              </thead>
              <tbody>
            ...
            {caseTab==='details' && (
              <div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
                  {[['שם לקוח',selectedCase.customer_name],['ארגון',selectedCase.org_name],['טלפון',selectedCase.phone],['ת״ז',selectedCase.id_number],['סיווג 1',selectedCase.cat1_name],['סיווג 2',selectedCase.cat2_name]].map(([l,v]) => v ? (
