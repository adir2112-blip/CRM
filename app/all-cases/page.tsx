      ...
      <div style={{ padding:'22px 26px' }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
          ...
          <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
            {Object.entries(DATE_RANGES).map(([k,v]) => (
              <button key={k} onClick={() => setFDate(k)} style={{ padding:'4px 12px', borderRadius:999, fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'Heebo,sans-serif', border:'none', background:fDate===k?'#2563eb':'#f1f3f8', color:fDate===k?'#fff':'#4b5568' }}>{v}</button>
            ))}
          </div>
        ...
        <div className="card" style={{ padding:0 }}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>#</th><th>שם לקוח</th><th>טלפון</th><th>ארגון</th><th>סיווג 1</th><th>סיווג 2</th><th>סטטוס</th><th>נציג</th><th>נוצר</th><th>עודכן</th></tr></thead>
              <tbody>
                ...
                )) : <tr><td colSpan={10} style={{ textAlign:'center', padding:'2rem', color:'var(--text3)' }}>אין פניות</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={e => { if(e.target===e.currentTarget) setSelectedCase(null) }}>
          <div className="modal">
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:16 }}>
              {statuses.map(s => { const st=CM[s.name]||{bg:'#f3f4f6',ab:'#6b7280',c:'#374151',b:'#d1d5db'}; const sel=editStatus===s.name; return <button key={s.id} onClick={() => setEditStatus(s.name)} style={{ padding:'7px 16px', borderRadius:8, fontSize:12, fontWeight:sel?700:500, cursor:'pointer', fontFamily:'Heebo,sans-serif', background:sel?st.ab:st.bg, color:sel?'#fff':st.c, border:`1.5px solid ${sel?st.ab:st.b}`, boxShadow:sel?`0 3px 10px ${st.ab}50`:'none', outline:'none' }}>{s.name}</button> })}
            </div>
            ...
            {caseTab==='sms' && <div>
              ...
              {smsTemplates.length>0 && <div className="form-group"><label className="form-label">בחר תבנית</label><select className="form-input" value={selectedTemplate} onChange={e => { setSelectedTemplate(e.target.value); const t=smsTemplates.find((x:any)=>x.id===e.target.value); if(t) setSmsText(t.content.replace('{שם}',selectedCase.customer_name)) }}><option value="">בחר תבנית...</option>{smsTemplates.map((t:any) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>}
              <div className="form-group"><label className="form-label">תוכן ההודעה</label><textarea className="form-input" rows={4} value={smsText} onChange={e => setSmsText(e.target.value)} placeholder="הקלד הודעה..." /><div style={{ fontSize:11, color:'var(--text3)', marginTop:4 }}>{smsText.length} תווים</div></div>
